...
import { getRecipe, listPatients, saveRecipe } from "@/lib/data/store";
...
function RecipeFormPageInner() {
  ...
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    saveRecipe({ id, patientId, title, notes });
    router.replace(`/patients/recipes${patientId ? `?patientId=${patientId}` : ""}`);
  }

  return (
    <div className="min-h-screen grid place-items-center p-6">
      <div className="w-full max-w-md rounded-3xl bg-[#C0C9EE] p-6 grid gap-3">
        <h1 className="text-lg">{id ? "Editar receita" : "Nova receita"}</h1>
        <form onSubmit={onSubmit} className="grid gap-3">
          <Autocomplete label="Paciente" options={patients.map((p) => ({ value: p.id, label: p.name }))} value={patientId} onChange={setPatientId} />
          <Input label="Titulo" placeholder="Nome da receita" value={title} onChange={(e) => setTitle(e.target.value)} />
          <label className="grid gap-2 text-sm">
            <span>Observações</span>
            <textarea className="min-h-[100px] rounded-[10px] bg-white/80 px-3 py-2 text-black" placeholder="Observações da receita" value={notes} onChange={(e) => setNotes(e.target.value)} />
          </label>
